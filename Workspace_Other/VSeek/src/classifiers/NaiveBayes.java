...
public class NaiveBayes {
    ...
    public String predict(String text) throws IllegalArgumentException {
        ...
        //System.out.println("Probability: " + maxScore + " : " + maxScoreCategory + " \n "+ text + "\n");
        
        return maxScoreCategory; //return the category with heighest score
    }
    
    public Map<String, Double> getRawScoresForEveryTopic(String text) throws IllegalArgumentException {
    	
    	Map<String, Double> topicScores = new HashMap<String, Double>();
    	if(knowledgeBase == null) {
            throw new IllegalArgumentException("Knowledge Bases missing: Make sure you train first a classifier before you use it.");
        }
        ...
        for(Map.Entry<String, Double> entry1 : knowledgeBase.logPriors.entrySet()) {
            ...
            topicScores.put(category, logprob); 
        ...
        return topicScores; //return the category with heighest score
    }
    
    
    
    /**
     * Returns a Map of all the topics and their scores
     * 
     * After all the scores have been given
     * 
     * I enter the category with the highest probability at the end of the map
     ...
     * @throws IllegalArgumentException
     */
    public Map<String, Double> getScoresForEveryTopicNormWithin(String text) throws IllegalArgumentException {
        ...
        //topicScores.put(maxScoreCategory, maxScore); 
//        System.out.println("Raw Scores From Classifier");
//        for (Map.Entry<String, Double> entry : topicScores.entrySet())
//		{
//		    System.out.println(entry.getKey() + ": " + entry.getValue());
//		}
        
        topicScores = NormalizeScoresWithin(topicScores);
        
        return topicScores; //return the category with heighest score
    }
    
    private Double NormalizeScore(Double score, Double max, Double min) {
     ...
     * 
     */
    private Map<String, Double> NormalizeScoresWithin (Map<String, Double> scores) {
    	
    	// Sorting all the scores produced for this topic
    	Collection<Double> scoreValues = scores.values();
    	List theList = new ArrayList(scoreValues);
    	...
    	// Use the sorted list to get the Max and Min values
    	Double max = (Double) theList.get(0);
    	Double min = (Double) theList.get(theList.size()-1);
    	
    	
    	Map<String,Double> normScores = new HashMap<String, Double>();
    	
    	// For each entry in Map, normalize score
    	for (Map.Entry<String, Double> entry : scores.entrySet())
    	{
    		String topic = entry.getKey();
    		Double rawScore = entry.getValue();
    		if (rawScore == Double.NEGATIVE_INFINITY) {
    			rawScore = min;
    		}
    	    Double score = NormalizeScore(rawScore, max, min);
    	    
    	    normScores.put(topic, score);
    	}
    	
    	// return new Map with normalized Scores
    	return normScores;
    }
    
    public Map<String, Double> NormalizeScoresThoughout (Map<String, Double> scores) {
    	...
    	{
    		String topic = entry.getKey();
    	    Double score = NormalizeScore(entry.getValue(), max, min);
    	    normScores.put(topic, score);
    	}
